...
export function generateInitialHeatmapData() {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    if (Math.random() < workChance) {
      ...
      heatmap[key] = { minutes, tasksCompleted, sessionsCount: Math.max(1, Math.floor(minutes / 30)) };
    } else {
...
/* ==========================================================================
   ...
   ========================================================================== */
function Header() {
  const { projects, heatmap, settings, setIsCommandPaletteOpen, setIsReelModalOpen, setIsApiKeyModalOpen, setIsNewProjectModalOpen, exportData, importData } = useAppState();
    ...
    <header className="sticky top-0 z-30 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/80 px-4 lg:px-8 py-3">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto justify-end">
          ...
          <button onClick={() => setIsApiKeyModalOpen(true)} className={`flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border ${settings.apiKey ? 'bg-emerald-950/40 text-emerald-300 border-emerald-800' : 'bg-amber-950/40 text-amber-300 border-amber-800'}`}>
            <Key className="w-3.5 h-3.5" />
            <span className="hidden lg:inline">{settings.apiKey ? 'GenAI Active' : 'Set API Key'}</span>
          </button>
          <button onClick={exportData} className="p-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-white">
            <Download className="w-4 h-4" />
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="p-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-white">
...
function AiResearchTile() {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      setTimeout(() => setLoadingStep('Executing google_search & url_context tools...'), 1000);
      setTimeout(() => setLoadingStep('Structuring task breakdown JSON...'), 2200);

      const result = await generateProjectBreakdown({ promptText, apiKey: settings.apiKey, sourceUrl: sourceUrl.trim() || null });
      ...
      <div className="flex flex-col space-y-4">
        ...
        {generatedResult && (
          <div className="p-4 rounded-xl bg-slate-900 border border-cyan-500/50 space-y-3">
            ...
            <button onClick={() => { addProject(generatedResult); setGeneratedResult(null); setPromptText(''); setSourceUrl(''); }} className="w-full flex items-center justify-center space-x-2 py-2 rounded-lg bg-emerald-500 text-slate-950 font-bold text-xs shadow-md shadow-emerald-500/20">
