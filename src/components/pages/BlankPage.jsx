...
export function BlankPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 lg:px-8 py-6 space-y-6 animate-in fade-in duration-200">
      ...
      {/* PAGE 2 TO PAGE 1 TRANSITION BAR */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900/90 to-indigo-950/60 border border-slate-800 shadow-2xl backdrop-blur-md">
        
        {/* Left: Page Transition Back Button */}
        <div className="flex items-center space-x-3">
          <button
            onClick={() => setCurrentView('page1')}
            className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-slate-950 font-bold text-xs transition shadow-lg shadow-cyan-500/20 group"
            title="Switch view to Page 1 Dashboard"
          >
            <ArrowLeft className="w-4 h-4 transition-transform group-hover:-translate-x-1" />
            <span>Switch to Page 1 (Main Dashboard)</span>
          </button>

          <span className="text-slate-700 hidden sm:inline">•</span>

          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-violet-950 text-violet-300 border border-violet-800">
              PAGE 2 WORKSPACE
            </span>
          </div>
        </div>

        {/* Right: Page 2 Navigation Sub-Tabs */}
        <div className="flex items-center space-x-1 bg-slate-950 p-1 rounded-xl border border-slate-800 text-xs overflow-x-auto">
          ...
          >
            ...
            <span>All Projects & Tasks ({projects.length})</span>
          </button>
